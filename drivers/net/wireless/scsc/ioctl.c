...
int slsi_set_bandwidth(struct net_device *dev, char *command)
{
	struct netdev_vif    *ndev_vif = netdev_priv(dev);
	struct slsi_dev      *sdev = ndev_vif->sdev;
	int bandwidth = 0;
	...
	if (!slsi_str_to_int(command, &bandwidth)) {
...
#define MAX_LEN_PRIV_COMMAND    4096 /*This value is the max reply size set in supplicant*/
	...
	} else if (strncasecmp(command, CMD_SETBANDWIDTH, strlen(CMD_SETBANDWIDTH)) == 0) {
		int skip = strlen(CMD_SETBANDWIDTH) + 1;

		ret = slsi_set_bandwidth(dev, command + skip);
#ifdef CONFIG_SCSC_WLAN_WIFI_SHARING
#if !defined(CONFIG_SCSC_WLAN_MHS_STATIC_INTERFACE) || (defined(SCSC_SEP_VERSION) && SCSC_SEP_VERSION < 90000)
